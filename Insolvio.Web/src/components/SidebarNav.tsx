...
import {
  ...
  FileText, Settings, LogOut, Upload, ChevronDown, BarChart3, Globe,
} from "lucide-react";
...
export default function SidebarNav({ onUploadClick }: SidebarNavProps) {
  ...
  <img src={logo} alt="Insolvio" className="h-8 w-auto rounded-md" />
 </div>
         ...
         <NavItem to="/settings" icon={Settings} label={t.nav.settings} />
          {isGlobalAdmin && (
            <>
              ...
              <NavItem to="/admin/regions" icon={Globe} label={t.nav.regions ?? "Regions"} />
            </>
 )}
           </>
        )}
