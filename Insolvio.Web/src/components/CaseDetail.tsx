    ...
    <div className="mx-auto max-w-5xl pb-12">
      ...
      <div className="mb-2">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
          {insolvencyCase.caseEmailAddress && (
            <>
      ...
      {documents.length > 0 && (
        <div className="mt-8">
          ...
          <select
            value={selectedDocId ?? ""}
            onChange={(e) =>
              setSelectedDocId(e.target.value ? e.target.value : null)
            }
            className="rounded-md border border-input bg-background px-3 py-2 text-sm text-foreground min-w-48"
          >
            <option value="">Select a document</option>
            {documents.map((doc) => (
              <option key={doc.id} value={doc.id}>
                {(doc.docType ?? "Document").replace(/_/g, " ")} –{" "}
                {doc.documentDate
                  ? typeof doc.documentDate === "string"
                    ? doc.documentDate
                    : ((doc.documentDate as {iso?: string; text?: string})
                        ?.iso ??
                      (doc.documentDate as {text?: string})?.text ??
                      "—")
                  : "—"}
              </option>
            ))}
