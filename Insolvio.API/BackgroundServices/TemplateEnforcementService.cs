...
db.ScheduledEmails.Add(new ScheduledEmail
 ...
 CaseId = letter.CaseId,
     To = adminEmail,
    Subject = $"[Insolvio] CRITICAL TEMPLATE {message} — {caseName} [{escalationKey}]",
  ...
  var caseName = letter.Case?.CaseNumber ?? "N/A";
   ...
   TenantId = letter.TenantId,
     CaseId = letter.CaseId,
        To = caseOwnerEmail,
    Subject = $"[Insolvio] URGENT: Template send deadline in {hoursRemaining:F0}h — {caseName} [{warningKey}]",
