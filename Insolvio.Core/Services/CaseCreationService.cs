...
$"from an uploaded notice document dated {noticeDate:dd MMM yyyy}. " +
 $"The system registered {partyCompanyMap.Count} companies, linked {partiesCreated} parties, " +
  ...
  private async Task<Insolvio.Domain.Enums.SystemRegion> GetTenantRegionAsync(Guid? tenantId)
  {
    if (!tenantId.HasValue) return Insolvio.Domain.Enums.SystemRegion.Romania;
    var tenant = await _db.Tenants.AsNoTracking()
      .FirstOrDefaultAsync(t => t.Id == tenantId.Value);
    return tenant?.Region ?? Insolvio.Domain.Enums.SystemRegion.Romania;
  }
      ...
      DateTime? nextHearing, string recipient)
  {
    ...
    AddReminder(claimsDeadline, 7,
      $"[Insolvio] Termen creanțe în 7 zile — {cas.DebtorName} ({cas.CaseNumber})",
    ...
    AddReminder(claimsDeadline, 1,
      $"[Insolvio] URGENT: Termen creanțe MÎINE — {cas.DebtorName}",
    ...
    AddReminder(nextHearing, 2,
      $"[Insolvio] Termen de judecată în 2 zile — {cas.DebtorName}",
    ...
    AddReminder(contestationsDeadline, 3,
      $"[Insolvio] Termen contestații în 3 zile — {cas.DebtorName}",
