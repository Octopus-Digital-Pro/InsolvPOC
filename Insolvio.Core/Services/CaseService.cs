...
public sealed class CaseService : ICaseService
{
     ...
     public CaseService(IApplicationDbContext db, ICurrentUserService currentUser, IAuditService audit, IFileStorageService storage, CaseCreationService caseCreation)
     {
          _db = db;
  ...
  .Select(d => d.ToDto())
     ...
     public async Task<CaseDto> CreateAsync(CreateCaseCommand command, CancellationToken ct)
     {
          ...
          if (insolvencyCase.CompanyId.HasValue)
          {
               ...
               var deadlineSettings = await _db.Set<Insolvio.Domain.Entities.TenantDeadlineSettings>()
                    .AsNoTracking()
                    .FirstOrDefaultAsync(s => s.TenantId == tenantId, ct);
               var reportIntervalDays = deadlineSettings?.ReportEveryNDays ?? 30;
               var reportTask = new Insolvio.Domain.Entities.CompanyTask
               {
                    Id = Guid.NewGuid(),
                    TenantId = tenantId,
                    ...
                    Status = Insolvio.Domain.Enums.TaskStatus.Open,
