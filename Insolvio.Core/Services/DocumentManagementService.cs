...
public sealed class DocumentService : IDocumentService
{
    ...
    public DocumentService(IApplicationDbContext db, ICurrentUserService currentUser, IAuditService audit)
    {
        _db = db;
    ...
    public async Task<DocumentDto> CreateAsync(CreateDocumentCommand cmd, CancellationToken ct)
    {
        ...
        var caseNumber = await _db.InsolvencyCases
            .Where(c => c.Id == cmd.CaseId && c.TenantId == tenantId)
            .Select(c => c.CaseNumber)
            .FirstOrDefaultAsync(ct);

        var summaries = LocalizedSummaryBuilder.BuildDocumentSummaryByLanguage(
            cmd.SourceFileName,
            cmd.DocType,
            caseNumber,
            null);
