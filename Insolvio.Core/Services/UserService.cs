...
public sealed class UserService : IUserService
{
    ...
    public UserService(IApplicationDbContext db, ICurrentUserService currentUser, IAuditService audit, IConfiguration config)
    {
        _db = db;
    ...
    public async Task<UserInvitationDto> InviteAsync(InviteUserRequest request, CancellationToken ct = default)
    {
        ...
        _db.ScheduledEmails.Add(new ScheduledEmail
        {
            ...
            To = invitation.Email,
            Subject = "Ați fost invitat(ă) să utilizați Insolvio",
...
<div style=""max-width:500px;margin:0 auto;background:#fff;border-radius:10px;padding:36px 40px;border:1px solid #e2e8f0"">
  <h2 style=""color:#1e293b;margin-top:0;font-size:22px"">Invitație Insolvio</h2>
  <p style=""color:#334155"">Bună <strong>{invitation.FirstName} {invitation.LastName}</strong>,</p>
  <p style=""color:#334155"">Ați fost invitat(ă) să accesați platforma <strong>Insolvio</strong> cu rolul <strong>{invitation.Role}</strong>.</p>
