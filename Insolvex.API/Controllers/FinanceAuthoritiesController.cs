...
{
    private readonly IFinanceAuthorityService _service;

    public FinanceAuthoritiesController(IFinanceAuthorityService service) => _service = service;
    ...
    public async Task<IActionResult> Create([FromBody] FinanceAuthorityRequest request, CancellationToken ct)
    {
    ...
    [HttpPut("{id:guid}")]
    [RequirePermission(Permission.SettingsEdit)]
    public async Task<IActionResult> Update(Guid id, [FromBody] FinanceAuthorityRequest request, CancellationToken ct)
