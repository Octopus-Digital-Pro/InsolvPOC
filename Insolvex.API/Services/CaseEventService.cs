...
public sealed class CaseEventService : ICaseEventService
{
    ...
    public CaseEventService(ApplicationDbContext db, ICurrentUserService currentUser)
    {
        _db = db;
    ...
    public async Task<CaseEventDto> RecordAsync(Guid caseId, RecordCaseEventRequest req, CancellationToken ct = default)
    {
        ...
        // Resolve active phase type if not supplied
        var phaseType = req.PhaseType ?? await GetActivePhaseTypeAsync(caseId, ct);
    ...
    public async Task<CaseEventDto> RecordPhaseEventAsync(Guid caseId,
        ...
        string? description, CancellationToken ct = default)
    {
        return await RecordAsync(caseId, new RecordCaseEventRequest
        {
            Category = "Phase",
            EventType = eventType,
            Description = description ?? (nextPhase != null
                ? $"Phase advanced: {HumanizePhase(completedPhase)} → {HumanizePhase(nextPhase)}"
                : $"Phase completed: {HumanizePhase(completedPhase)}"),
            PhaseType = completedPhase,
            Severity = "Info",
            Metadata = new { completedPhase, nextPhase },
        }, ct);
    }
    ...
    public async Task<List<CaseEventDto>> GetByCaseAsync(Guid caseId, int page = 1, int pageSize = 50,
    ...
    private async Task<string?> GetActivePhaseTypeAsync(Guid caseId, CancellationToken ct)
    {
        try
        {
            return await _db.CasePhases
                .Where(p => p.CaseId == caseId && p.Status == Domain.Enums.PhaseStatus.InProgress)
                .Select(p => p.PhaseType.ToString())
    ...
    private static CaseEventDto ToDto(CaseEvent ev) => new(
        ...
        ev.PhaseType, ev.Severity, ev.MetadataJson
    );
    ...
    private static string HumanizePhase(string phase) =>
        System.Text.RegularExpressions.Regex.Replace(phase, "([A-Z])", " $1").Trim();
}
