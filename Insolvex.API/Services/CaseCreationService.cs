...
// Step 1: Create/find Company records
 ...
 CourtName = courtName,
   ...
   DebtorCui = parties.FirstOrDefault(p => p.Role == "Debtor")?.FiscalId,
   ...
   tasks.Add(CreateTask(tenantId, companyId, cas.Id, CaseStage.EligibilitySetup,
    $"Generate initial notices (Templates-Ro) — {cas.DebtorName}",
      ...
      $"Notificare deschidere procedur? — {cas.DebtorName}",
          $"Publicare în BPI ?i notificare ONRC pentru dosarul {cas.CaseNumber}.",
...
tasks.Add(CreateTask(tenantId, companyId, cas.Id, CaseStage.FormalNotifications,
        ...
        tasks.Add(CreateTask(tenantId, companyId, cas.Id, CaseStage.AssetAssessment,
            ...
            $"Întocmire raport privind cauzele ?i împrejur?rile care au dus la apari?ia st?rii de insolven?? — dosar {cas.CaseNumber}.",
   ...
   tasks.Add(CreateTask(tenantId, companyId, cas.Id, CaseStage.CreditorClaims,
      $"Verificare ?i întocmire tabel preliminar crean?e — {cas.DebtorName}",
...
$"[Insolvex] Termen crean?e în 7 zile — {cas.DebtorName} ({cas.CaseNumber})",
            $"Termenul limit? pentru depunerea crean?elor în dosarul {cas.CaseNumber} ({cas.DebtorName}) este {claimsDeadline?.ToString("dd.MM.yyyy")}.\n\nMai ave?i 7 zile.");

        AddReminder(claimsDeadline, 1,
            $"[Insolvex] URGENT: Termen crean?e MÂINE — {cas.DebtorName}",
     $"ATEN?IE: Termenul pentru crean?e în dosarul {cas.CaseNumber} expir? MÂINE {claimsDeadline?.ToString("dd.MM.yyyy")}.");

        // Hearing reminder
        AddReminder(nextHearing, 2,
         $"[Insolvex] Termen de judecat? în 2 zile — {cas.DebtorName}",
  ...
  AddReminder(contestationsDeadline, 3,
      $"[Insolvex] Termen contesta?ii în 3 zile — {cas.DebtorName}",
