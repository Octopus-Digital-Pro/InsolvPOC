...
public sealed class CaseService : ICaseService
{
     ...
     public CaseService(ApplicationDbContext db, ICurrentUserService currentUser, IAuditService audit, IFileStorageService storage)
     {
          _db = db;
  ...
  .FirstOrDefaultAsync(x => x.Id == id && (tenantId == null || x.TenantId == tenantId), ct)
    ?? throw new BusinessException($"Case {id} not found.");
       ...
       : $"Case '{c.CaseNumber}' details were updated.";
          ...
          await _audit.LogAsync(new AuditEntry
          {
               Action = oldStage != c.Stage ? "Case Stage Transitioned" : "Case Details Updated",
